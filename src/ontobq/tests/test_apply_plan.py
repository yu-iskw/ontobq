...
from ontobq.orchestrate.plan import DeploymentPlan, build_plan
...
def _offline_plan() -> DeploymentPlan:
...
def test_commerce_apply_submits_views_before_graph_verbatim() -> None:
    ...
    assert tuple(item.status for item in result.artifacts) == ("applied",) * len(plan.artifacts)
...
def test_second_apply_same_plan_is_unchanged_and_adds_no_targets() -> None:
    ...
    assert first.ok
    assert second.ok
    assert tuple(item.status for item in second.artifacts) == ("unchanged",) * len(plan.artifacts)
...
def test_apply_cli_commerce_success(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert statuses == ["applied"] * len(statuses)
    assert len(fake.submitted) == len(statuses)
