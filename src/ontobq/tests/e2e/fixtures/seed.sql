-- TEMPLATE — substitute ${ONTOBQ_E2E_PROJECT} and ${ONTOBQ_E2E_DATASET} before running.
...
CREATE OR REPLACE TABLE `${ONTOBQ_E2E_PROJECT}.${ONTOBQ_E2E_DATASET}.e2e_customers` AS
SELECT * FROM UNNEST([
...
CREATE OR REPLACE TABLE `${ONTOBQ_E2E_PROJECT}.${ONTOBQ_E2E_DATASET}.e2e_orders` AS
SELECT * FROM UNNEST([
  STRUCT(
...
CREATE OR REPLACE TABLE `${ONTOBQ_E2E_PROJECT}.${ONTOBQ_E2E_DATASET}.e2e_products` AS
SELECT * FROM UNNEST([
...
CREATE OR REPLACE TABLE `${ONTOBQ_E2E_PROJECT}.${ONTOBQ_E2E_DATASET}.e2e_order_items` AS
SELECT * FROM UNNEST([
  STRUCT(
    'ord_1001' AS order_id,
    'SKU_W' AS sku,
