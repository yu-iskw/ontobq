...
the `ontobq` console script exposes `validate`, `plan`, and `apply`.
...
- **CLI**: `ontobq validate` and `ontobq plan` are read-only. `ontobq apply`
  builds a #15 plan then mutates through `MutationExecutor`. Default
  validate/plan runs layers 1–4 and needs BigQuery clients; `--offline` runs
  load + semantics only and does not construct clients; `--skip-integrity`
  skips data scans. `apply` always uses layers 1–4.
